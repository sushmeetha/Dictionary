...
import dictionary.SortedDictionaryCounting;
...
public class DictionarySorting {
	...
	public static void main(String[] args)
	{
		...
		String text1=SortedDictionaryCounting.Text;
		Map<String, Integer> wordOccurrenceMap = GetWordOccurrenceMap(text1);

		PrintWordOccurrenceCount(wordOccurrenceMap);

	}
	
	private static Map<String, Integer> GetWordOccurrenceMap(String text)

	{
...

		{
			...
			if (!dictionary.StringHelper.isNullOrEmpty(word.trim()))

			{

				int count;

				if (!(words.containsKey(word) ? (count = words.get(word)) == count : false))
	...
	private static void PrintWordOccurrenceCount(Map<String, Integer> wordOccurenceMap)

	{

		for (Map.Entry<String, Integer> wordEntry : wordOccurenceMap.entrySet())

		{

			System.out.printf("Word '%1$s' occurs %2$s time(s) in the text" + "\r\n", wordEntry.getKey(), wordEntry.getValue());
